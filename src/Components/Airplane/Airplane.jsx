...
import { useBook } from '../../Context/BookContext'; // import your book context hook

export default function Airplane({ selectedCategoryId }) {
  const [events, setEvents] = useState([]);
  const [loadingEvents, setLoadingEvents] = useState(false);
  const [pageIndex, setPageIndex] = useState(1);
  const [bookingLoadingIds, setBookingLoadingIds] = useState([]); // Track loading per event ID
  ...
  const handleBook = async (event) => {
    ...
    setBookingLoadingIds((prev) => [...prev, event.id]); // Mark this event as loading
    ...
    setBookingLoadingIds((prev) => prev.filter((id) => id !== event.id)); // Remove loading state
  };
    ...
    <div className="mb-12">
      ...
      <div className="sm:flex items-center justify-center gap-8 px-8 py-4">
        ...
        <div className="sm:w-1/2 mt-5 sm:mt-0">
          <h2 className="text-3xl font-bold mb-4">Book Your Flight Today</h2>
          <p className="text-lg text-gray-700">
            Discover amazing destinations and book your airplane ticket easily with our seamless booking system.
            Make your trip unforgettable with comfort and convenience from the start.
          </p>
        </div>
      </div>

      <h1 className="text-3xl font-bold text-center mb-6 mt-20">Events</h1>
                ...
                >
                  ...
                  <div className="text-center p-4">
                    ...
                    <button
                      ...
                      className="mt-2 bg-blue-500 text-white p-2 rounded-md hover:bg-blue-400 cursor-pointer"
                      disabled={isLoading}
                    >

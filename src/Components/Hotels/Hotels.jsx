...
import backgroundhotel from '../../assets/backgroundhotels.jpg';
import { useBook } from '../../Context/BookContext';  // <-- import your booking context hook

export default function Hotels({ selectedCategoryId }) {
  const [events, setEvents] = useState([]);
  const [loadingEvents, setLoadingEvents] = useState(false);
  const [pageIndex, setPageIndex] = useState(1);
  const [bookingLoadingIds, setBookingLoadingIds] = useState([]); // Track booking loading per event
  ...
  const fetchEvents = async (categoryId = 7, page = 1) => {
  ...
  useEffect(() => {
    fetchEvents(selectedCategoryId || 7, pageIndex);
  ...
  const handleBook = async (event) => {
    ...
    const result = await createBook({
      eventId: event.id,
      categoryId: selectedCategoryId || 7,
    });
    ...
    <div className="px-6 mb-12">
      ...
      <div className="sm:flex items-center justify-center gap-8 px-8 py-4">
        <div className="sm:w-1/2">
          <img className="w-full h-[400px] object-cover rounded-md shadow" src={backgroundhotel} alt="Hotels" />
        </div>
        <div className="sm:w-1/2 mt-5 sm:mt-0">
          <h2 className="text-3xl font-bold mb-4">Book Your Hotel Today</h2>
          <p className="text-lg text-gray-700">
            Discover amazing places and book your hotel easily with our seamless booking system.
            Make your stay unforgettable with comfort and convenience from the start.
          </p>
        </div>
      </div>

      <h1 className="text-3xl font-bold text-center mb-6 mt-20">Hotel Events</h1>
